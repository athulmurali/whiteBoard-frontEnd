export  const COURSE_SEC_API_BASE_URL = 'https://mean-exam-manager-backend.herokuapp.com';
export  const  SECTION_URL = '/api/course/courseId/section';
export  const  GET_SECTION_SUFIX = '/api/section/sectionId';
export const   ADD_SECTION_SUFFIX = '/api/course/courseId/section';
export const   DEL_SECTION_SUFFIX = '/api/course/courseId/section/sectionId';

// course Id and user token to be given
export const   GET_ENROLLED_SECTION = '/api/course/courseId/enrolledSectionId';

export const  ENROLL_STUDENT_SUFFIX = '/api/student/username/section/sectionId';

export  const WAIT_TIME = 3000;

